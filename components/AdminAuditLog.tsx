...
import { History, RefreshCw, RotateCcw, Trash2, PlusCircle, Pencil, AlertCircle, ArrowLeft, CheckCircle2, CalendarDays } from 'lucide-react';
...
const HoldToRestoreButton: React.FC<{ onConfirm: () => void; isLoading: boolean }> = ({ onConfirm, isLoading }) => {
  ...
  return (
    <button
      ...
      disabled={isLoading}
      className="relative overflow-hidden inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-900 text-white text-xs font-bold rounded-lg hover:bg-black transition-all shadow-sm disabled:opacity-50 group select-none"
    >
...
export const AdminAuditLog: React.FC<AdminAuditLogProps> = ({ onBack }) => {
  ...
  const handleRestore = async (log: AuditLogRecord) => {
    ...
    try {
      // 1. DATA RECOVERY: Extract 'shifts' from either direct object (if triggered by legacy system) 
      //    OR from the injected '_archived_shifts' inside details (Frontend Soft-Delete pattern)
      const oldData = log.old_data;
      
      // Look for shifts in various places
      let shiftsToRestore = oldData.shifts || oldData.details?._archived_shifts || [];
      
      // 2. CLEANUP: Prepare Shopper Data
      // Remove the temporary archive fields so we don't pollute the new record
      ...
      // Important: Remove 'shifts' key from root if it exists, as it's not a column in 'shoppers' table
      ...
      // 4. INSERT SHIFTS (if any were recovered)
      if (Array.isArray(shiftsToRestore) && shiftsToRestore.length > 0) {
          // Prepare shifts (ensure they point to the restored shopper ID)
          const shiftsPayload = shiftsToRestore.map((s: any) => ({
              shopper_id: shopperPayload.id, // Explicitly link to restored ID
              date: s.date,
              time: s.time,
              type: s.type
          }));
      ...
      // Success feedback
      const shiftCount = shiftsToRestore.length;
      alert(`${shopperPayload.name} restored successfully with ${shiftCount} shifts!`);
  ...
  const getOpBadge = (op: string) => {
    switch (op) {
      case 'INSERT': return <span className="flex items-center gap-1 bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold border border-green-200"><PlusCircle className="w-3 h-3" /> CREATED</span>;
      case 'UPDATE': return <span className="flex items-center gap-1 bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-200"><Pencil className="w-3 h-3" /> EDITED</span>;
      case 'DELETE': return <span className="flex items-center gap-1 bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold border border-red-200"><Trash2 className="w-3 h-3" /> DELETED</span>;
      default: return <span className="bg-gray-100 text-gray-700 px-2 py-1 rounded text-xs font-bold">{op}</span>;
    }
  };
  ...
  const renderDiff = (log: AuditLogRecord) => {
      ...
      // Check if we have shifts in the backup (via the new _archived_shifts logic)
              ...
              <span className="font-bold text-gray-800 flex items-center gap-2">
                  ...
                  {shiftBackupCount > 0 && (
                      <span className="text-[10px] bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded flex items-center gap-1 font-normal border border-purple-200" title="Includes archived shifts">
                          <CalendarDays className="w-3 h-3" /> {shiftBackupCount} Shifts
                      </span>
                  )}
              </span>
      ...
      <div className="flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">View history and restore deleted records (including shifts).</p>
          </div>
        </div>
        <div className="flex gap-2">
           ...
           <button onClick={fetchLogs} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500 transition-colors">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3 text-sm text-blue-800">
          ...
          <div>
              ...
              <p className="mt-1 opacity-90 leading-relaxed">
                  Hold button for 1.5s to restore. <br/>
                  The system now attempts to recover both <strong>Profile + Shifts</strong> automatically.
              </p>
          </div>
      </div>
    </div>
