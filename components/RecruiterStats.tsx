...
import { Users, TrendingUp, Award, CalendarCheck, Loader2, Calendar, Filter } from 'lucide-react';
...
export const RecruiterStats: React.FC<RecruiterStatsProps> = ({ staffList }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-4 md:px-6 py-4 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/50">
            <div className="flex items-center gap-3">
                <div className="p-2 bg-purple-100 text-purple-600 rounded-lg">
                    <TrendingUp className="w-5 h-5" />
                </div>
                <div>
                    <h3 className="font-bold text-gray-900 text-sm uppercase tracking-wider">Team Performance</h3>
                    <p className="text-xs text-gray-500">Recruitment velocity & shift impact</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full md:w-auto bg-white p-1 rounded-lg border shadow-sm">
                ...
                <select 
                    ...
                    className="bg-transparent text-xs font-bold text-gray-700 outline-none py-1.5 pr-2 cursor-pointer w-full md:w-48"
                >
                    ...
                    {availableWeeks.map(dateStr => {
                        ...
                        return (
                            <option key={dateStr} value={dateStr}>
                                {isCurrent ? 'Current Week' : label} ({format(start, 'd MMM')} - {format(end, 'd MMM')})
                            </option>
                        );
                    })}
                </select>
        ...
        <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {stats.map((recruiter, index) => {
                    ...
                    const percentage = (recruiter.hires / maxHires) * 100;
