...
export const DesktopDayPanel: React.FC<DesktopDayPanelProps> = ({
  ...
  const visibleShifts = SHIFT_TIMES.filter(shift => {
      // In AA Mode or Admin, show everything (AA logic handles disabled)
      if (mode === 'ADMIN' || (mode === 'SHOPPER' && step === 0 && !isFWDSelection)) return true;
      ...
      // UX IMPROVEMENT: Hide unavailable shifts in Standard mode
        ...
        <div className="p-6 space-y-4 overflow-y-auto bg-white">
          ...
          {visibleShifts.length === 0 ? (
             <div className="flex flex-col items-center justify-center py-10 text-center opacity-70">
                 ...
                 <p className="font-bold text-gray-500">No shifts available for this day.</p>
             </div>
          ) : (
            visibleShifts.map((shift) => {
              ...
              if (mode === 'SHOPPER') {
                  if (isFWDSelection) {
                      if (!stdAvailable) { isActionDisabled = true; disabledReason = null; } 
